"""Guard: build_operator_engagement_store must not silently fall back to in-memory."""
...
from agent_runtime.store import InMemoryOperatorEngagementStore, PostgresOperatorEngagementStore
...
def test_default_raises_without_database_url() -> None:
    ...
    with patch.dict("os.environ", {}, clear=True):
        with pytest.raises(AgentRuntimeConfigError, match="Postgres operator engagement store required"):
...
def test_allow_in_memory_returns_in_memory_with_warning(caplog: pytest.LogCaptureFixture) -> None:
    ...
    assert isinstance(store, InMemoryOperatorEngagementStore)
    assert any(
        "OPERATOR_ENGAGEMENT_STORE_FALLBACK_TO_MEMORY" in rec.message for rec in caplog.records
    )
...
def test_postgres_when_settings_url_present() -> None:
    ...
    assert isinstance(store, PostgresOperatorEngagementStore)


def test_postgres_when_agent_runtime_env_url_present() -> None:
    settings = SimpleNamespace(mailbox_memory_database_url="")
    with patch.dict(
        "os.environ",
        {"MAILBOX_MEMORY_DATABASE_URL": "postgresql://user:pass@127.0.0.1:5432/mailbox"},
