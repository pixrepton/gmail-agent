"""Guard: build_turn_journal_for_settings must not silently fall back to in-memory."""
...
from daszek_engagement_feed import build_turn_journal_for_settings
...
def test_allow_in_memory_returns_in_memory_with_warning(caplog: pytest.LogCaptureFixture) -> None:
    ...
    assert any("AGENT_TURN_JOURNAL_STORE_FALLBACK_TO_MEMORY" in rec.message for rec in caplog.records)
