"""Smoke: case_guidance stage wired to output_model with fallback envelope."""
...
def test_case_guidance_output_model_validates() -> None:
    settings = _minimal_settings()
    payload = {
        "operational_status": "active_review",
        "waiting_for": "client",
        "reason_summary_pl": "Czekamy na dane.",
        "blocker_summary_pl": "",
        "momentum": "steady",
        ...
        "next_step_hint_pl": "Sprawdź wątek.",
        "confidence": 0.6,
    }
    fake_stage = {
    ...
    parsed = CaseGuidanceResult.model_validate(out["response_json"])
    assert isinstance(parsed, CaseGuidanceResult)


def test_case_guidance_output_model_failure_returns_envelope() -> None:
    settings = _minimal_settings()
    bad_payload = {"operational_status": "active_review", "confidence": 0.5}
    fake_stage = {
        "stage_name": "case_guidance",
    ...
    with pytest.raises(ValidationError):
        CaseGuidanceResult.model_validate(out["response_json"])
