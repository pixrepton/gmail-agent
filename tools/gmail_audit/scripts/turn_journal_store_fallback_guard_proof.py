#!/usr/bin/env python3
"""Proof: build_turn_journal_for_settings guard against silent InMemory fallback.

Audits all callers and runs guard unit tests.

Stdout on success: TURN_JOURNAL_STORE_FALLBACK_GUARD_PROOF_OK
"""
...
from daszek_engagement_feed import build_turn_journal_for_settings  # noqa: E402

PRODUCTION_CALLERS: dict[str, str] = {
    "daszek_engagement_feed/__init__.py": "build_engagement_feed_for_cel; default allow_in_memory=False",
}

TEST_PATCH_CALLERS = {
    "tests/test_daszek_engagement_feed_pr_e_complete.py": "patch build_operator_engagement_store; patch journal in feed test",
}
...
def _find_call_sites(root: Path) -> list[dict[str, str | int | bool]]:
    ...
    for path in sorted(root.rglob("*.py")):
        ...
        for node in ast.walk(tree):
            ...
            if name != "build_turn_journal_for_settings":
                continue
...
def _run_guard_unit_tests() -> None:
    tests = Path(__file__).resolve().parents[1] / "tests" / "test_turn_journal_store_fallback_guard.py"
    proc = subprocess.run(
...
def _assert_guard_behavior() -> dict[str, object]:
    ...
    with patch.dict("os.environ", {}, clear=True):
        try:
            build_turn_journal_for_settings(settings)
            raise AssertionError("expected AgentRuntimeConfigError without database URL")
        except AgentRuntimeConfigError:
            pass
        journal = build_turn_journal_for_settings(settings, allow_in_memory=True)
...
def _audit_callers(sites: list[dict[str, str | int | bool]]) -> dict[str, object]:
    ...
    for site in sites:
        ...
        if rel.endswith("turn_journal_store_fallback_guard_proof.py"):
            continue
        if site.get("is_test"):
        ...
        for key in PRODUCTION_CALLERS:
            if key in rel.replace("\\", "/"):
                ...
                matched = True
                break
        if not matched and "daszek_engagement_feed" in rel:
            production_hits.append(label)
            matched = True
        if not matched:
    ...
    if unknown:
        raise RuntimeError(f"unclassified build_turn_journal_for_settings callers: {unknown}")
...
def main() -> int:
    report: dict[str, object] = {"proof": "turn_journal_store_fallback_guard", "ok": False}
    try:
        ...
        print("TURN_JOURNAL_STORE_FALLBACK_GUARD_PROOF_OK")
        return 0
    except Exception as exc:  # noqa: BLE001
        report["error"] = str(exc)
        print(json.dumps(report, ensure_ascii=True, indent=2), file=sys.stderr)
        print(f"TURN_JOURNAL_STORE_FALLBACK_GUARD_PROOF_FAIL: {exc}", file=sys.stderr)
        return 1
